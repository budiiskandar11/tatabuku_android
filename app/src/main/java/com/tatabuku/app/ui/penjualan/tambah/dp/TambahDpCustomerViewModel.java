...
public class TambahDpCustomerViewModel extends ViewModel {
    ...
    public TambahDpCustomerViewModel() {
    ...
    public void addDP(Integer customerId, Integer value, Boolean isRefund, Date date, Integer dpId) {
        if (value == 0) {
            ...
            if (dpId != 0) {
                updateDP(new DPPostModel(dpId, customerId, value, dateString, selectedBank.getValue().getId()));
            } else if (isRefund) {
                submitRefundDP(new AddDPModel(customerId, value, dateString, selectedBank.getValue().getId()));
            } else {
                submitAddDP(new AddDPModel(customerId, value, dateString, selectedBank.getValue().getId()));
            }
        }
    }
    ...
    private void submitAddDP(AddDPModel model) {
        Call<SubmitOrderResponse> call = ConnectionManager.getInstance().getService().addDPCustomer(model);
        call.enqueue(new Callback<SubmitOrderResponse>() {
            @Override
            public void onResponse(Call<SubmitOrderResponse> call, Response<SubmitOrderResponse> response) {
                if (response.body() != null) {
    ...
    private void submitRefundDP(AddDPModel model) {
        Call<SubmitOrderResponse> call = ConnectionManager.getInstance().getService().refundDPCustomer(model);
        call.enqueue(new Callback<SubmitOrderResponse>() {
            @Override
            public void onResponse(Call<SubmitOrderResponse> call, Response<SubmitOrderResponse> response) {
                if (response.body() != null) {
    ...
    public void fetchRekeningData(int customerId, int dpId) {
        RekeningDataModel model = new RekeningDataModel(customerId);
