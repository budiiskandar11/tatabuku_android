...
public class CheckoutReturViewModel extends ViewModel {
    ...
    public void saveRetur() {
        ...
        ReturnOrderParams orderParams = new ReturnOrderParams(list.toArray(new ReturnModel[0]), supplierResult.getValue().getId());
        ReturnOrderModel returnOrderModel = new ReturnOrderModel(orderParams);
        Call<SubmitOrderResponse> call = ConnectionManager.getInstance().getService().submitRetur(returnOrderModel);
