...
import com.tatabuku.app.databinding.ActivityCheckoutPenjualanBinding;
...
import com.tatabuku.app.ui.pembelian.checkout.CheckoutPembelianListAdapter;
...
public class CheckoutPenjualanActivity extends BaseActivity {

    private ActivityCheckoutPenjualanBinding binding;
    private CheckoutPenjualanListAdapter listAdapter;
    private LinearLayoutManager layoutManager;
    private CheckoutPenjualanViewModel viewModel;
    ...
    private void setupViewModel() {
        viewModel = new ViewModelProvider(this).get(CheckoutPenjualanViewModel.class);
    }
...
//                binding.supplierPhone.setText(customerDetailTotalData.getPhone());
                try {
                    Glide.with(CheckoutPenjualanActivity.this).asBitmap()
                ...
                binding.receiveAddress.setText(customerDetailTotalData.getStreet() + " " + customerDetailTotalData.getStreet2());
//                binding.receivePhone.setText(customerDetailTotalData.getPhone());
            }
        });
        viewModel.getOrderDetailResult().observe(this, new Observer<OrderDetailPenjualanResult>() {
            @Override
            public void onChanged(OrderDetailPenjualanResult orderDetailResult) {
                hideLoading();
                binding.date.setText(StringHelper.formatReceivedDate(orderDetailResult.getOrderDate()));
                binding.orderNo.setText(String.format(getString(R.string.order_no_n), orderDetailResult.getOrderNo()));
                listAdapter.notifyDataSetChanged();
            }
        });
        ...
        viewModel.getOnSuccess().observe(this, new Observer<Integer>() {
            ...
            public void onChanged(Integer id) {
                hideLoading();
                if (id == 0) {
                    Intent intent = new Intent(CheckoutPenjualanActivity.this, DetailCustomerActivity.class);
                    intent.putExtra(DetailCustomerActivity.ARG_CUSTOMER_ID, viewModel.getCustomerData().getValue().getId());
                    intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    startActivity(intent);
                } else {
                    Intent intent = new Intent(CheckoutPenjualanActivity.this, PengirimanBarangActivity.class);
                    intent.putExtra(PengirimanBarangActivity.ARG_ORDER_ID, id);
                    intent.putExtra(PengirimanBarangActivity.ARG_CUSTOMER, viewModel.getCustomerData().getValue());
                    startActivity(intent);
                }
            }
        });
    ...
    private void setupView() {
        binding = ActivityCheckoutPenjualanBinding.inflate(getLayoutInflater());
    ...
    private void configureView() {
        ...
        binding.addItem.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(CheckoutPenjualanActivity.this, OrderPenjualanActivity.class);
                intent.putExtra(OrderPenjualanActivity.ARG_ORDER_ID, orderId);
                intent.putExtra(OrderPenjualanActivity.ARG_CUSTOMER, viewModel.getCustomerData().getValue());
    ...
    private void setupList() {
        ...
        listAdapter = new CheckoutPenjualanListAdapter(viewModel, this);
